...
#   Character.create(name: 'Luke', movie: movies.first)
 ...
 100.times do
   ...
   reviewed_coffee_shops = []

   # Generate between 1 to 5 unique reviews per customer
   rand(1..5).times do
     coffee_shop = CoffeeShop.order('RANDOM()').first

     # Ensure the customer hasn't already reviewed this coffee shop
     unless reviewed_coffee_shops.include?(coffee_shop.id)
       Review.create!(
         customer: customer,                      # Assign the customer
         coffee_shop: coffee_shop,                # Assign the random coffee shop
         rating: rand(1..5),                      # Generate a random rating between 1 and 5
         comment: Faker::Coffee.notes             # Generate coffee notes as a comment
       )
       reviewed_coffee_shops << coffee_shop.id
     end
   end
 end
